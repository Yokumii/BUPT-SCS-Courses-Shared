...
def iter_documents(data_dir: str, verbose: bool = True, limit: int = 0):
    """
    流式解析 data_dir 下所有 .txt 文件，按需产出有效 Document。

    参数:
        data_dir: 数据目录路径
        verbose: 是否显示解析进度
        limit: 最多返回多少篇有效文档，0 表示全部

    Yields:
        经过过滤并重新编号后的 Document
    """
    ...
    for i, filename in enumerate(files):
        ...
        if verbose and (i + 1) % 2000 == 0:
            print(
                f"  已解析 {i + 1}/{total} 个文件，"
                f"有效: {valid_count}，跳过: {skipped}"
            )

    if verbose:
        seen = min(total, valid_count + skipped) if limit > 0 else total
        print(
            f"解析完成: 共 {seen} 个文件，"
            f"有效文档: {valid_count}，跳过: {skipped}"
        )


def parse_all_documents(data_dir: str, verbose: bool = True, limit: int = 0) -> list:
    """
    ...
    """
    return list(iter_documents(data_dir, verbose=verbose, limit=limit))
